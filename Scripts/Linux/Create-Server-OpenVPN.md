# Installer un serveur OpenVPN avec un script Shell

## 📖 Description

Cette procédure permet d'installer automatiquement un serveur **OpenVPN** à l'aide d'un script Shell (`.sh`).

Le script réalise l'installation et la configuration d'OpenVPN de manière interactive.

---

## 📋 Prérequis

- Distribution Linux compatible (Debian, Ubuntu… selon le script).
- Compte disposant des privilèges `sudo`.
- Connexion Internet.
...
Coller le contenu du script dans l'éditeur ci-dessous : 
...
```bash
sudo chmod +x <chemin_du_script>
```

Exemple :

```bash
sudo chmod +x /home/admin/install-openvpn.sh
```
...
```bash
sudo /home/admin/install-openvpn.sh
```

Suivre ensuite les différentes étapes proposées par le script.

---

# 🔍 Vérification

Une fois l'installation terminée, vérifier que le service OpenVPN est actif :
...
- Télécharger le script uniquement depuis une source fiable.
- Lire son contenu avant de l'exécuter.
- Conserver une copie du script pour une réinstallation ultérieure.
- Vérifier que le port utilisé par OpenVPN (généralement **UDP 1194**) est autorisé sur le pare-feu.
